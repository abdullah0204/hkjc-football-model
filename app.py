...
def search_leagues(keyword, league_list):
    ...
    fuzzy = get_close_matches(keyword, league_list, n=20, cutoff=0.2)
...
def analyse_group(df, goals_col, line):
    over_rate = (df[goals_col] > line).mean()
    under_rate = 1 - over_rate

    return {
        "matches": len(df),
        "avg_goals": df[goals_col].mean(),
        "median_goals": df[goals_col].median(),
        "over_rate": over_rate,
        "under_rate": under_rate,
        "over_1_5": (df[goals_col] > 1.5).mean(),
        "over_2_5": (df[goals_col] > 2.5).mean(),
        "over_3_5": (df[goals_col] > 3.5).mean(),
        "over_4_5": (df[goals_col] > 4.5).mean()
    }
...
def recent_trend(df, league_col, goals_col, date_col, selected_league, line):
    if date_col is None:
    ...
    for days in [30, 90, 180]:
        ...
        rows.append({
            ...
            f"Under {line}": f"{(1 - over_rate) * 100:.1f}%"
        })
...
def make_bet_log_row(bet_date, league, home, away, line, decision, side, over_odds, under_odds, stake):
    if side == "Over":
        odds = over_odds
    elif side == "Under":
        odds = under_odds
    ...
    if "result" in df.columns:
        ...
        for _, row in df.iterrows():
            if pd.notna(row.get("profit_loss")):
                calculated.append(row.get("profit_loss"))
                continue
...
def show_bet_log(df):
    ...
    total_bets = len(df)
    wins = (df["result"].astype(str).str.lower() == "win").sum() if "result" in df.columns else 0
    losses = (df["result"].astype(str).str.lower() == "lose").sum() if "result" in df.columns else 0
    pushes = (df["result"].astype(str).str.lower() == "push").sum() if "result" in df.columns else 0
...
main_url = st.sidebar.text_input("Main Database CSV URL", value=SHEET_CSV_URL)
bet_log_url = st.sidebar.text_input("Bet Log CSV URL", value=BET_LOG_CSV_URL)
...
try:
    bet_log_df = load_csv(bet_log_url)
except Exception:
...
with tab_model:
    ...
    home_input = st.sidebar.text_input("Home Team", value="坎培拉祖雲達斯")
    away_input = st.sidebar.text_input("Away Team", value="昆比恩城")

    home_choice = st.sidebar.selectbox("Suggested Home Team", get_team_suggestions(home_input, team_list))
    away_choice = st.sidebar.selectbox("Suggested Away Team", get_team_suggestions(away_input, team_list))
    ...
    if analyse_button:
        ...
        trend_rows, signal = recent_trend(df, league_col, goals_col, date_col, selected_league, line)
        ...
        bet_row, chosen_odds = make_bet_log_row(
            bet_date,
            selected_league,
            display_home,
            display_away,
